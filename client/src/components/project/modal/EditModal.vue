        ...
        <b-field grouped>
          ...
          >
            <b-autocomplete
              ...
              v-model="searchClient.query.name"
              clearable
              open-on-focus
              append-to-body
...
export default {
  ...
  data() {
    return {
      project: {
        name: this.data.name,
        client: this.data.client ? this.data.client._id : '',
        team: this.data.team._id,
        estimate: this.data.estimate
          ? new Date(this.data.estimate)
          : new Date(),
  ...
  methods: {
    async store() {
      ...
      try {
        await projectApi.update(this.data._id, this.project)
  ...
  mounted() {
    if (this.data.client) {
      this.searchClient.query.name = this.data.client.name
    }

    this.searchTeam.query.name = this.data.team.name
  },
}
</script>
